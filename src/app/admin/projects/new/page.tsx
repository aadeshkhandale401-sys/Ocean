    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500">Upload project details, site photos, and videos.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-xs space-y-1">
            <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3">Project Details</h3>
            <div className="input-group">
              <label>Project Title *</label>
              <input name="title" value={form.title} onChange={handleChange} required className="input-field" placeholder="e.g., MGPS & ICU Setup — City Hospital" />
            </div>
            ...
            <div className="input-group">
              ...
              <textarea name="description" value={form.description} onChange={handleChange} rows={4} className="input-field" placeholder="Scope of work..." />
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-xs space-y-4">
            <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center gap-2">
              <ImageIcon size={14} className="text-blue-600" /> Site Photos ({images.length})
            </h3>
            {images.length > 0 && (
          ...
          <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-xs space-y-4">
            <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center gap-2">
              <Video size={14} className="text-indigo-600" /> Site Videos ({videos.length})
            </h3>
            {videos.length > 0 && (
            ...
            <label className="flex flex-col items-center gap-2 p-5 rounded-xl border-2 border-dashed border-slate-200 cursor-pointer hover:border-indigo-500 hover:bg-indigo-50/20 transition-all">
              ...
              <span className="text-xs font-medium text-slate-500">
                {uploading ? `Uploading... ${Math.round(progress)}%` : "Upload Video File (MP4, WEBM)"}
              </span>
            ...
            <div className="flex items-center gap-2">
              <input type="url" value={videoUrlInput} onChange={(e) => setVideoUrlInput(e.target.value)}
                placeholder="Or paste YouTube / video URL..." className="input-field text-xs" />
              <button type="button" onClick={handleAddVideoUrl} className="btn btn-outline btn-sm whitespace-nowrap flex-shrink-0">
                <Plus size={14} /> Add
              </button>
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          <button type="submit" disabled={saving} className="btn btn-primary w-full">
            {saving ? <><span className="spinner spinner-sm" /> Saving...</> : <><Save size={16} /> Save Project</>}
          </button>
        </div>
      </form>
    </div>
